...
import android.os.EnvironmentHidden;
...
import net.sb418.android.gmscompatx.patches.DynamiteClientPatches;
...
import notjava.io.FileDescriptorHidden;
...
public final class GmsDynamiteClientHooks {
	...
	public static void maybeInit(String auth) {
		...
		synchronized (GmsDynamiteClientHooks.class) {
			...
			int userId = Refine.<ContextHidden>unsafeCast(GmsCompat.appContext()).getUserId();
			String deDataDirectory = EnvironmentHidden.getDataUserDeDirectory(null, userId).getPath();
		...
		// install Dynamite client hooks
		DynamiteClientPatches.INSTANCE.install();
	}

	// ApkAssets#loadFromPath(String, int, AssetsProvider)
	public static @Nullable ApkAssets loadAssetsFromPath(@NonNull String path, int flags, AssetsProvider assets)
			throws IOException {
	...
	// File#lastModified()
	public static long getFileLastModified(@NonNull File file) {
		final String path = file.getPath();

		if (enabled && path.startsWith(gmsCoreDataPrefix)) {
			String fdPath = "/proc/self/fd/" + Refine.<FileDescriptorHidden>unsafeCast(modulePathToFd(path)).getInt$();
	...
	public static String maybeModifyClassLoaderPath(String path, boolean nativeLibsPath) {
		if (path == null) {
			return null;
		}
		...
		for (int i = 0; i < pathParts.length; ++i) {
			...
			String fdFilePath = "/gmscompat_fd_" + Refine.<FileDescriptorHidden>unsafeCast(modulePathToFd(filePath)).getInt$();
	...
	private static @NonNull FileDescriptor modulePathToFd(String path) {
		if (DEBUG) {
