...
import android.app.usage.StorageStatsHidden;
...
import net.sb418.android.gmscompatx.patches.PlayStorePatches;
...
public final class PlayStoreHooks {
	...
	public static void init() {
		...
		// install Play Store patches
		PlayStorePatches.INSTANCE.install();
	}

	// PackageInstaller#createSession
	public static void adjustSessionParams(@NonNull PackageInstaller.SessionParams sessionParams) {
		PackageInstallerHidden.SessionParams params = Refine.unsafeCast(sessionParams);
	...
	static class PackageInstallerStatusForwarder extends BroadcastReceiver {
		...
		public void onReceive(Context receiverContext, @NonNull Intent intent) {
			...
			int status = extras.getInt(PackageInstaller.EXTRA_STATUS, Integer.MIN_VALUE);

			if (status == PackageInstaller.STATUS_PENDING_USER_ACTION) {
				Intent confirmationIntent = intent.getParcelableExtra(Intent.EXTRA_INTENT, Intent.class);

				String packageName = null;

				if (extras.containsKey(PackageInstaller.EXTRA_SESSION_ID)) {
					int sessionId = extras.getInt(PackageInstaller.EXTRA_SESSION_ID, Integer.MIN_VALUE);
	...
	public static void deletePackage(PackageManager pm, String packageName, IPackageDeleteObserver observer,
	                                 int flags) {
		if (flags != 0) {
		...
		Refine.<ContextHidden>unsafeCast(GmsCompat.appContext()).getMainThreadHandler().postDelayed(() -> {
			try {
				// Play Store ignores this callback as of version 33.6.13, but provide it anyway
				// in case it's fixed
				observer.packageDeleted(packageName, PackageManagerHidden.DELETE_FAILED_ABORTED);
			} catch (RemoteException e) {
	...
	public static StorageStats queryStatsForPackage(String packageName) throws NameNotFoundException {
		String apkPath = packageManager.getApplicationInfo(packageName, ApplicationInfoFlags.of(0)).sourceDir;

		var stats = new StorageStatsHidden();
		stats.codeBytes = new File(apkPath).length();
		// leave dataBytes, cacheBytes, externalCacheBytes at 0
		return Refine.unsafeCast(stats);
	}
