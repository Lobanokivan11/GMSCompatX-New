...
public class StubDef implements Parcelable {
	...
	public static StubDef find(StackTraceElement[] stackTrace, GmsCompatConfig config, int mode) {
		...
		if (mode == FIND_MODE_Parcel) {
			...
			// GMSCompatX: our hook on `Parcel.readException` adds extra entries between the two
			// `Parcel.readException` entries, depending on what hook framework is in-use

			// find second `android.os.Parcel.readException`
			StackTraceElement ste;
			for (firstIndex = 3; firstIndex < stackTrace.length; firstIndex++) {
				ste = stackTrace[firstIndex];
				if (Objects.equals(ste.getClassName(), "android.os.Parcel")
					&& Objects.equals(ste.getMethodName(), "readException")) {
					firstIndex++;
					break;
				}
			}
	...
	private static final byte[] CONST_EMPTY_BYTE_ARRAY = new byte[] {};
	private static final int[] CONST_EMPTY_INT_ARRAY = new int[] {};
	private static final long[] CONST_EMPTY_LONG_ARRAY = new long[] {};
	...
	public boolean stubOutMethod(Parcel p) {
		...
		switch (type) {
			...
			case EMPTY_BYTE_ARRAY:
				p.writeByteArray(CONST_EMPTY_BYTE_ARRAY);
				break;
			case EMPTY_INT_ARRAY:
				p.writeIntArray(CONST_EMPTY_INT_ARRAY);
				break;
			case EMPTY_LONG_ARRAY:
				p.writeLongArray(CONST_EMPTY_LONG_ARRAY);
				break;
			case EMPTY_STRING:
				p.writeString("");
				break;
